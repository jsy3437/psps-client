...
- 메인페이지 : 메인배너버튼 브라우저 크기에 맞춰서 길이 줄어들게 수정
- 결제페이지 : 상품상세페이지에서 이동시 상품데이터 들어오지 않는 현상
  _전에 상품페이지에서 상세페이지 이동했다가 돌아올 때 카테고리 유지를 위해 useEffect return에 history state에 replace 적용을 했지만 결제페이지로 이동시에도 state가 바뀌어서 history.action이 POP일때만 replace적용을 해둠_
...
- images : 이미지 압축 후 변경 완료
- Footer : 전체페이지에 들어가서 App.js로 이동 완료
- 주문결과페이지 : 로컬, 배포 페이지 주소 다른거 env, config 추가 완료
- 탈퇴페이지 : 탈퇴문구, 스타일링 수정 완료
- 마이페이지 : 주문내역 대표이미지 수정 완료

---

> 2022.03.28

- 상품상세페이지 : 옵션리스트 스타일링 변경사항 수정

---

> 2022.03.27

- 상품상세페이지 : 스크롤시 해당 스크롤컨트롤러에 focus 구현완료
- 상품상세페이지 : 장바구니 담기 클릭시 옵션 초기화 안되게 수정
- 클레임페이지 : 환불신청으로 바꿨다가 교환신청으로 오면 사유적어도 확인버튼 활성화안되는 오류 수정

---

> 2022.03.26

- 마이페이지 : 주문상세내역 주문삭제 요청코드 구현완료
- 마이페이지 : 주문내역 취소, 교환, 환불 요청 변경사항 적용
- 클레임페이지 : 신청사유 항상 직접입력으로 레이아웃 변경

---

> 2022.03.25

- 마이페이지 : 주문상세내역 배송조회 팝업 구현완료
...
- Footer : 내부 문구 일부 수정
- Footer : 이용약관 클릭시 반응없는 현상, 이용약관 title 수정
- 고객센터페이지 : 최초 렌더링시 스크롤 최상단으로 이동하게 수정
- 메인페이지 : 추천상품 디자인 변경사항 수정
- 상품페이지 : 상품리스트 디자인 변경사항 수정
- 소개페이지 : 소개글 data폴더에 정리
- 마이페이지 : 해당 주문내역의 상품이 2개 이상일때 상품 이미지에 그림자 박스 생기게 수정
- 마이페이지 : 비밀번호변경 에러메세지 오류 수정

---

> 2022.03.21

- 상품상세페이지 : OrderBox 옵션리스트박스 스크롤 스타일 수정
- 상픔상세페이지 : 장바구니 담기 버튼 클릭시 말풍선 alert 구현완료
- 상품페이지 : 상품상세페이지로 이동후 다시 돌아올때 카테고리 유지시키는 코드 구현완료
- 상품페이지 : 페이지 이동으로 접근시 페이지 상단으로 스크롤 실행되게 구현완료
...
- 회원탈퇴페이지 : 문구 수정
- 마이페이지 : 메뉴 중복 클릭시 해당 컴포넌트 최상위로 이동하게 수정
...
- 마이페이지 : 회원탈퇴페이지로 연결완료
- 회원탈퇴페이지 : 레이아웃, 스타일링, 기능, 서버요청 완료

---

> 2022.03.17

- 장바구니페이지 : 디자인 변경사항 수정완료
- 마이페이지 : 개인정보변경 컴포넌트들 버튼 비활성화, 활성화일때 나누어 적용
- 상품상세페이지 : OrderBox의 상품이미지 디자인 수정
- 마이페이지 : 주문내역 리스트 pointer로 변경, 가짜버튼생성

---

> 2022.03.16

- 찾기변경페이지 : 비밀번호 찾기 변경요청 후 서버응답처리완료
- 상품상세페이지 : 스크롤 컨트롤러 화면 고정완료, infoTable의 내용량/중량 조건 넣어서 띄우기 완료
- 찾기페이지 : 아이디, 비밀번호 찾기 버튼 활성화 전 cursor 수정 완료
- 회원가입페이지 : step1,step3 버튼 활성화 상태일때만 호버 수정완료
- 마이페이지 : 카드영수증 확인 버튼 클릭시 new 브라우저 사이즈 조정
- 전체페이지 : 디자인 border-radius 변경된거 수정완료

---

> 2022.03.15

- 찾기페이지 : 비밀번호 찾기 인증 후 새 비밀번호 입력페이지 레이아웃, 스타일링 완료
- 찾기페이지 : 비밀번호 찾기 데이터 받고 서버로 보내는 기능까지 완료 => 서버에러 수정 후 한번 더 테스트 해보기
- 브랜드소개페이지 : 폰트 디자인 변경사항 수정
- 전체페이지 : 디자인 border-radius 전부 변경되어서 수정중

---

> 2022.03.14

- .env : 로컬, 배포상태의 env 분리 후 자동으로 적용되게 수정
- controller : 분리된 env를 이용해 로컬과 배포상태의 요청주소가 자동으로 변경되게 수정
- 메인페이지 : 메인배너, 서브배너 cursor pointer로 수정
- SubBanner : 배너 클릭시 해당 요소의 주소로 이동하게 수정

---

> 2022.03.13

- eslint : console에 warning 정리
- 메인페이지 : 서브배너 슬라이드 스타일링 수정, 이미지 크기 맞지 않을때 스크롤 위치 오류 수정 => 자동으로 사이즈 조절
- 장바구니페이지 : 상품체크변동시 공급원별 가격이 0원일때 해당 리스트 사라지게 수정완료
...
- 방병도(Front), 조서윤(Front), 최도영(Back), 최준영(Design)
